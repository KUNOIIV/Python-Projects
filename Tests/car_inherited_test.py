...
def test_inheritance():
    # Test inheritance: SportsCar gets Car's methods + adds turbo
    my_ride = SportsCar("Audi R8", 60)  # Create SportsCar (calls super().__init__)
    my_ride.start_engine()               # Inherited method from Car
    assert my_ride.engine_running        # Verifies inherited attribute True after start
    assert hasattr(my_ride, "turbo")     # SportsCar adds 'turbo' attribute
    assert not hasattr(Car("Audi TTRS", 60), "turbo")  # Base Car lacks turbo
